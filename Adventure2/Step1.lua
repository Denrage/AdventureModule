...
function poisonedTabletConversation()
    local dialog = Dialog:Create("test1");
...
function tabletConversation()
    local dialog = Dialog:Create("test2");
...
function wallConversation()
    local dialog = Dialog:Create("test3");
...
function teapotConversation()
    local dialog = Dialog:Create("test4");
...
function gamliConversation()
    local dialog = Dialog:Create("test5");
    ...
    dialog.AddEdge(1, 2, "Was befindet sich hinter der Wand dahinten?", function() : bool return ClientVariables.interactedWithWall end);
    dialog.AddEdge(1, 3, "Woher kommt der Punch?", function() : bool return ClientVariables.interactedWithTablet end);
...
function robbConversation()
    local dialog = Dialog:Create("test6");
    dialog.AddNode(1, "Hallo");
    ...
    dialog.AddEdge(1, 2, "Woher kommt der Eierpunsch?", function() : bool return ClientVariables.interactedWithTablet end);
    dialog.AddEdge(1, -1, "Auf Wiedersehen");
    dialog.AddEdge(2, -1, "Auf Wiedersehen");
...
function nathanConversation()
    local dialog = Dialog:Create("test7");
    dialog.AddNode(1, "Hallo");
    ...
    dialog.AddEdge(1, 2, "Was hattest du mit dem Punch gemacht?", function() : bool return ClientVariables.interactedWithTablet end);
    dialog.AddEdge(1, 3, "Gamli meinte, du hättest den Schlüssel für den Laggerraum, könnte ich diesen haben?", function() : bool return ClientVariables.talkedWithGamliKey end);
    dialog.AddEdge(1, -1, "Auf Wiedersehen");
    dialog.AddEdge(2, -1, "Auf Wiedersehen");
...
function natashaConversation()
    local dialog = Dialog:Create("test8");
    dialog.AddNode(1, "Hallo");
    dialog.AddEdge(1, -1, "Auf Wiedersehen");
